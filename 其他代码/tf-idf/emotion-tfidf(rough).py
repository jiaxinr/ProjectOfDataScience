...
def calTFIDF(inputdir, dirname):  # 根据语料库目录计算每一个词的词频和TF-IDF
    ...
    i = 0  # 总文档数
    positive = getListFromTxt("积极.txt")
    negative = getListFromTxt("消极.txt")
    neutral =getListFromTxt("中立.txt")
    for doc in documents:
        ...
        for x in doc:  # 统计每个词的词频
            if x in positive:
                if not (isInFile.get("积极", False)): #如果这个词是在所有文档中第一次出现，把他加入进去并默认置为false；如果这个词在这个文档中是第一次出现，把他加进去并默认置为false
                    isInFile["积极"] = True
                    txt_freq["积极"] = txt_freq.get("积极", 0) + 1  # 如果出现在某个文档中这个词在文档中的出现数目+1
                id_freq["积极"] = id_freq.get("积极", 0) + 1
            if x in negative:
                if not (isInFile.get("消极", False)):
                    isInFile["消极"] = True
                    txt_freq["消极"] = txt_freq.get("消极", 0) + 1
                id_freq["消极"] = id_freq.get("消极", 0) + 1
            if x in neutral:
                if not (isInFile.get("中立", False)):
                    isInFile["中立"] = True
                    txt_freq["中立"] = txt_freq.get("中立", 0) + 1
                id_freq["中立"] = id_freq.get("中立", 0) + 1
            else:
                pass
...
class TFIDF(object):
    ...
    def extract_keywordsInCorpus(self, topK=3):
...
if __name__ == '__main__':
    ...
    idffile = dirname + ".txt"
    tfidf = TFIDF(idffile)
    select_words, topK = tfidf.extract_keywordsInCorpus(topK=3)  # 获得的topK个关键词
